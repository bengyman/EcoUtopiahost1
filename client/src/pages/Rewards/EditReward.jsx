...
const EditReward = () => {
  ...
  useEffect(() => {
    const fetchReward = async () => {
      try {
        const response = await axios.get(`/reward/${id}`);
  ...
  const handleUpdateReward = async () => {
    ...
    try {
      await axios.put(`/reward/${id}`, formData, {
        headers: {
  ...
  return (
    <Container size="sm" my="5%">
      <Title align="center" mb="2rem">Edit Reward</Title>
      <Box
        sx={{
          display: 'flex',
      ...
      >
        ...
        >
          ...
          <Group position="apart" mt="2rem" grow>
            ...
            <Button type="submit" style={{ flexGrow: 1 }}>Update Reward</Button>
          </Group>
        </form>
      </Box>
    </Container>
